    ...
    public function loadMoreProducts()
    {
        ...
        if (CSRFToken::verifyCSRFToken($request->token, false)) {
            ...
            if (isset($request->subcategory)) {
                $subcategory1 = SubCategory::findBySlug($request->subcategory);
                $subcategory = SubCategory::findBySlugAndCategoryId($request->slug, $category->id);
            }
//var_dump($subcategory->category_id); die();
            $item_per_page = $count + $request->next;
            if ($subcategory && $category) {
                $products = Product::where('category_id', $category->id)
                    ->where('sub_category_id', $subcategory1->id)
