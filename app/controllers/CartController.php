...
class CartController extends BaseController
{
    ...
    public function addItem()
    {
        if (Request::has('post')) {
            ...
            if (CSRFToken::verifyCSRFToken($request->token, false)) {
                ...
                Cart::add($request);
                echo json_encode(['success' => '<a href="/cart" class="cart-notification">Product Added to Cart Successfully</a>']);
                exit;
            }
        }
    }
        ...
        if (Request::has('post')) {
            ...
            try {
                ...
                $charge = Charge::create([
                    ...
                    'amount' => $amount,
                    'description' => user()->full_name . '-cart purchase',
                    'currency' => 'aud'
                ]);

                //$this->logPaymentAndMailClient('stripe', $charge);

                $order_id = strtoupper(uniqid('pcf4ct0ry-', true));
                foreach ($_SESSION['user_cart'] as $cart_items) {
                    $productId = $cart_items['product_id'];
                    $quantity = $cart_items['quantity'];
                    $item = Product::where('id', $productId)->first();
            ...
            Cart::clear();
            echo json_encode([
                'success' => 'Thank you, we have received your payment. We are now processing your order.'
            ]);
        }
    }
    ...
    {
        Cart::clear();
        echo json_encode(['success' => 'Your cart is empty']);
        exit;
    }
